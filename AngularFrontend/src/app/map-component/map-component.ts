...
export class MapComponent implements OnInit {
  ...
  constructor(){
    ...
    effect(() => {
      ...
      if(this._static_data_editing_service.editing_station() && this.map) {
        const marker = this.station_marker_map.get(editing_station!.StaticStationData.id);
        if(marker && marker.dragging) {
          marker.on("drag", () => {
            const latlng = marker.getLatLng();
            this._static_data_editing_service.editing_station.set({
              ...this._static_data_editing_service.editing_station()!,
              StaticStationData: {
                ...this._static_data_editing_service.editing_station()!.StaticStationData,
                latitude: latlng.lat,
  ...
  updateMarkers(){
      ...
      if(stations && this.map) {
        ...
        stations.forEach(station => {
          const marker = L.marker([station.StaticStationData.latitude, station.StaticStationData.longitude],{draggable: true, icon: icon({...L.Icon.Default.prototype.options,iconUrl: 'assets/marker-icon.png',iconRetinaUrl: 'assets/marker-icon-2x.png',shadowUrl: 'assets/marker-shadow.png'})}).addTo(this.station_marker_group)
